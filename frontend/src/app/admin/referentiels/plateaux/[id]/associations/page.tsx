...
import { plateauAPI, PlateauTechnique, Hopital } from "@/lib/api";
...
export default function PlateauAssociationsPage() {
  const router = useRouter();
  const params = useParams();
  const plateauId = parseInt(params.id as string);

  const [plateau, setPlateau] = useState<PlateauTechnique | null>(null);
  ...
  const loadData = async () => {
    try {
      ...
      const plateauRes = await plateauAPI.getOne(plateauId);
      setPlateau(plateauRes.data);
      ...
      const associationsRes = await api.get(
        `/plateau-technique/associations/plateau/${plateauId}/`
      );
  ...
  const handleAddSelected = async () => {
    ...
    try {
      setSaving(true);
      await plateauAPI.associateHopitaux(plateauId, {
  ...
  const handleRemoveSelected = async () => {
    ...
    if (!confirm(`Retirer ${selectedIds.length} hôpital(x) de ce plateau ?`))
      return;
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-6">
          ...
          <h1 className="text-3xl font-bold text-gray-900">
            Associations - {plateau?.nom}
          </h1>
          <p className="text-gray-600 mt-2">
            Gérez les hôpitaux associés à ce plateau technique
          </p>
        </div>
