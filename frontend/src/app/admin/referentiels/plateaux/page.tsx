...
import { plateauTechniqueAPI, PlateauTechnique } from "@/lib/plateauTechnique";
import { PlusIcon, XIcon, AlertIcon, UploadIcon } from "@/components/Icons";

export default function ReferentielPlateauxPage() {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      await plateauTechniqueAPI.create({ nom: newName.trim() });
      setSuccess("Plateau technique ajouté avec succès");
      setNewName("");
      setShowForm(false);
      loadPlateaux();
    } catch (err: any) {
      if (err.response?.status === 400) {
        setError("Ce plateau technique existe déjà");
      } else {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm("Supprimer ce plateau technique ?")) return;

    try {
      await plateauTechniqueAPI.delete(id);
      setSuccess("Plateau technique supprimé");
      loadPlateaux();
    } catch (err: any) {
    ...
    <div className="p-6">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-slate-900 dark:text-slate-100">
          Référentiel - Plateaux Techniques
        </h1>
        <p className="mt-2 text-slate-600 dark:text-slate-400">
          Gérez la liste des plateaux techniques disponibles dans le système
        </p>
      </div>
        ...
        <div className="p-6 border-b border-slate-200 dark:border-slate-700">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">
              Liste des plateaux techniques ({plateaux.length})
            </h2>
        ...
        <div className="p-6">
          {plateaux.length === 0 ? (
            <div className="text-center py-12 text-slate-500 dark:text-slate-400">
              Aucun plateau technique dans le référentiel
            </div>
          ) : (
            <div className="space-y-2">
              {plateaux.map((plateau) => (
                ...
                >
                  ...
                  <button
                    onClick={() => handleDelete(plateau.id)}
