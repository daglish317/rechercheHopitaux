...
import { examenAPI, ExamenMedical } from "@/lib/examen";
import { PlusIcon, XIcon, AlertIcon, UploadIcon } from "@/components/Icons";

export default function ReferentielExamensPage() {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      await examenAPI.create({ nom: newName.trim() });
      ...
      if (err.response?.status === 400) {
        setError("Cet examen existe déjà");
      } else {
    ...
    <div className="p-6">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-slate-900 dark:text-slate-100">
          Référentiel - Examens Médicaux
        </h1>
        <p className="mt-2 text-slate-600 dark:text-slate-400">
          Gérez la liste des examens disponibles dans le système
        </p>
      </div>
        ...
        <div className="p-6">
          {examens.length === 0 ? (
            <div className="text-center py-12 text-slate-500 dark:text-slate-400">
              Aucun examen dans le référentiel
            </div>
          ) : (
