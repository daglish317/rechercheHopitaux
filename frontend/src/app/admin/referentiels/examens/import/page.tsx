...
export default function ImportExamensPage() {
  ...
  const handleImport = async () => {
    ...
    try {
      const response = await examenAPI.importExcel(file);
      setResult({
        success: true,
  ...
  const handleDownloadTemplate = () => {
    ...
    link.href = url;
    link.setAttribute("download", "template_examens.csv");
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-4xl mx-auto">
        <div className="mb-6">
          ...
          <h1 className="text-3xl font-bold text-gray-900">Importer des examens médicaux</h1>
          <p className="text-gray-600 mt-2">
            Importez une liste d'examens depuis un fichier Excel ou CSV
          </p>
        </div>
              ...
              <div className="flex-1">
                ...
                {result.success ? (
                  <div className="text-sm text-green-800 space-y-1">
                    <p>{result.message}</p>
                    <p>✓ {result.created} examen(s) créé(s)</p>
                    <p>⊘ {result.skipped} examen(s) ignoré(s) (doublons)</p>
                  </div>
                ) : (
                ...
                {result.success && (
                  <button
                    onClick={() => router.push("/admin/referentiels/examens")}
