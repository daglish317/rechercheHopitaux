...
import { examenAPI, ExamenMedical, Hopital } from "@/lib/api";
...
export default function ExamenAssociationsPage() {
  const router = useRouter();
  const params = useParams();
  const examenId = parseInt(params.id as string);

  const [examen, setExamen] = useState<ExamenMedical | null>(null);
  ...
  const loadData = async () => {
    try {
      ...
      const examenRes = await examenAPI.getOne(examenId);
      ...
      const associationsRes = await api.get(
        `/examens/associations/examen/${examenId}/`
      );
  ...
  const handleAddSelected = async () => {
    ...
    try {
      setSaving(true);
      await examenAPI.associateHopitaux(examenId, {
  ...
  const handleRemoveSelected = async () => {
    ...
    if (!confirm(`Retirer ${selectedIds.length} hôpital(x) de cet examen ?`))
      return;
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-6">
          ...
          <h1 className="text-3xl font-bold text-gray-900">
            Associations - {examen?.nom}
          </h1>
          <p className="text-gray-600 mt-2">
            Gérez les hôpitaux associés à cet examen
          </p>
        </div>
